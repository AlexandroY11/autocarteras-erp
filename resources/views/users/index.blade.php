    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="bg-white rounded-[2.5rem] p-6 border border-gray-100 shadow-sm hover:shadow-md transition-all group relative overflow-hidden">
            ...
            <div class="mt-6 pt-4 border-t border-gray-50 flex justify-end gap-2">
                ...
                @if($user->id !== auth()->id())
                <form x-data id="delete-form-{{ $user->id }}" method="POST" action="/users/{{ $user->id }}">
                    @csrf @method('DELETE')
                    <button type="button" 
                        @click="showAlert.delete('delete-form-{{ $user->id }}', {{ \Illuminate\Support\Js::from('¿Deseas eliminar al usuario ' . $user->name . '?') }})"
                        class="p-2.5 bg-gray-50 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all cursor-pointer">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
