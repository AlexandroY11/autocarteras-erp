...
<div class="pt-4 space-y-5" x-data="{ showFilters: false }">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="bg-white rounded-[2.5rem] p-5 border border-gray-100 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
            
            {{-- Decoración de color según estado/zona --}}
            ...
            <div class="flex gap-4 relative">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex justify-between items-start">
                        <div>
                            <h3 class="font-bold text-gray-900 text-lg leading-tight truncate">{{ $client->full_name }}</h3>
                            <a href="https://wa.me/57{{ preg_replace('/[^0-9]/', '', $client->phone ) }}" target="_blank" class="inline-block">
                                <p class="text-blue-600 font-bold text-sm hover:underline">{{ $client->phone }}</p>
                            </a>
                        </div>
                        @if(!$client->active)
                            <span class="bg-red-50 text-red-600 text-[10px] font-black uppercase px-2 py-1 rounded-lg">Inactivo</span>
                        @endif
                    </div>

                    {{-- BLOQUE DE INFORMACIÓN --}}
                    <div class="mt-4 space-y-3">
                        {{-- Dirección (Ancho completo para evitar cortes) --}}
                        <div class="flex items-start gap-3 text-gray-600">
                            ...
                            <div class="min-w-0">
                                <p class="text-[10px] font-bold text-gray-400 uppercase leading-none mb-1">Dirección de entrega</p>
                        ...
                        {{-- Grid inferior para Zona y Fecha --}}
                        <div class="grid grid-cols-2 gap-3 pt-1">
                            <div class="flex items-center gap-2 text-gray-500">
                                <div class="p-1.5 bg-gray-50 rounded-lg shrink-0">
                                ...
                                <div class="min-w-0">
                                    <p class="text-[10px] font-bold text-gray-400 uppercase leading-none">Zona</p>
                            ...
                            <div class="flex items-center gap-2 text-gray-500">
                                ...
                                <div class="min-w-0">
                                    <p class="text-[10px] font-bold text-gray-400 uppercase leading-none">Registro</p>
                                    <p class="text-[11px] font-semibold text-gray-600 truncate">{{ $client->created_at->format('d/m/Y') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="mt-5 pt-4 border-t border-gray-50 flex justify-end gap-2">
                <a href="/clients/{{ $client->id }}/edit" class="p-2.5 bg-gray-50 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-colors">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                ...
                <form id="delete-form-{{ $client->id }}" method="POST" action="/clients/{{ $client->id }}">
                    @csrf @method('DELETE')
                    <button type="button" 
                        @click="confirmDelete('delete-form-{{ $client->id }}', 'El cliente {{ $client->full_name }} será eliminado permanentemente.')"
